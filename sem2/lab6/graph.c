...
Node* makeNode(const char *name, double x, double y) {
...
void removeFromList(ListNode **list, Node *node) {  
    ListNode *prev = NULL;
...
void Search(Node *node, int *cycle, struct Path *list) { 
    if (*cycle) {
        return;
    }
    node->color = 'g';
    ...
    while (listNode) {
        ...
        if (listNode->node->color == 'w') {
            Search(listNode->node, cycle, list);
        }
